...
#Check Backup
/usr/bin/restic -r ${BACKUP_LOCATION} check | lolcat -t 

/usr/bin/restic -q  -r ${BACKUP_LOCATION} unlock # Unlock repo

/usr/bin/restic -q -r ${BACKUP_LOCATION} forget --keep-last 14 --prune | lolcat -t # Only keep 14 days

# Backup the system
/usr/bin/restic -r ${BACKUP_LOCATION} backup ${HOME} --exclude="${HOME}/.cache" --cleanup-cache=true --exclude-file=${HOME}/bin/resticExcludes.txt | lolcat -t
